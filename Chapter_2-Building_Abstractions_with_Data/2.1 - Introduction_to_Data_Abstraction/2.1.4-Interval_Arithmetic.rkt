...
;(center (make-center-width 3 1))


;;2.12 - make center percent
(define (make-center-percent center percentage)
  (let ((tolerance (* center (/ percentage 100))))
    (make-interval (- center tolerance) (+ center tolerance))))

(define (percent interval)
  (let* ((width (/ (- (upper-bound interval) (lower-bound interval)) 2))
         (mid-value (+ (lower-bound interval) width)))
    (* (/ width mid-value) 100)))

;(display "percent selector")
;(percent (make-center-percent 20 50))


;;2.13 - multiplication width
(define (mul-width a b)
  (let ((mid-a (+ (lower-bound a) (/ (- (upper-bound a) (lower-bound a)) 2)));; 2
        (mid-b (+ (lower-bound b) (/ (- (upper-bound b) (lower-bound b)) 2))));; 4
    (- (* mid-a mid-b) (* (lower-bound a) (lower-bound b)))))
    
(mul-width (make-center-percent 10 10) (make-center-percent 20 10))
